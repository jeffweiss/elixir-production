...
- **Simple architectures first**: A Python monolith on Postgres scaled to a $1.7B valuation with 70 engineers (Luu, "In defense of simple architectures"). Complexity burden should concentrate on unavoidable domain challenges — not infrastructure you don't yet need. The right question isn't "will this scale?" but "do we have evidence this won't scale?" Distributed architectures, microservices, and event sourcing are tools for problems you've measured, not insurance against problems you imagine.
